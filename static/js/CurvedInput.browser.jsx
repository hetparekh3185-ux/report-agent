// Browser build of CurvedInput — for use with <script type="text/babel">
// and the React/ReactDOM CDN globals. This is the SAME component as
// CurvedInput.jsx; the only changes are:
//   1. No `import`/`export` — those need a bundler (webpack/Vite/etc.)
//      which this plain dashboard.html doesn't have. Hooks are pulled off
//      the global `React` object instead.
//   2. It attaches itself to `window.CurvedInput` so dashboard.html's
//      mount script can find it.
// If you later move this into a Next.js/Vite/CRA project, use
// CurvedInput.jsx instead (the normal ES module version) and delete this
// file — it's a static-HTML-only workaround, not the source of truth.
...
const supportsDOMPoint = typeof window !== 'undefined' && typeof window.DOMPoint === 'function';
...
function CurvedInput({
  ...
  const handleSurfaceClick = e => {
    ...
    const textEl = textRef.current;
    if (supportsDOMPoint && svg && geom && layout && textEl && display.length) {
      try {
