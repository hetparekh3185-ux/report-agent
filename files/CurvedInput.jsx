...
// `ltr` = true when this edge segment is being drawn left-to-right.
const edgeSeg = (g, uTo, v, ltr) => {
  if (g.straight) return `L ${fmt(g, uTo, v)}`;
  const rho = round2(g.R - g.dir * v);
  // Parens added for clarity only — `>` already binds tighter than `===`,
  // so this was already evaluating as `ltr === (g.dir > 0)`.
...
// Some environments (older WebViews, certain test/SSR harnesses) don't
// implement DOMPoint. Rather than let a click silently throw its way into
// the catch block every time, fail soft to "no supported point transform"
// once, up front.
...
def fetch_content(topic: str, num_pages: int) -> str:
    ...
    try:
        ...
        raise GroqRequestError(f"Error fetching content from Groq: {exc}") from exc inputMode={inputMode}
        name={name}
        value={val}
